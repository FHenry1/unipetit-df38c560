      ...
      <div className="-mt-6 space-y-5 px-5">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <QuickAction
            ...
            label="Avaliações"
            sub={newReviews ? `${newReviews} novas` : "Ver todas"}
            onClick={() => navigate({ to: "/owner/reviews" })}
          />
          <QuickAction
            icon={<LayoutDashboard size={18} />}
            label="Página pública"
            sub="Como o cliente vê"
            onClick={() => navigate({ to: "/snackbar/$id", params: { id: mySnackbar.id } })}
          />
        </div>
...
function EditSnackbarModal({
  ...
  const [openingTime, setOpeningTime] = useState(snackbar.opening_time?.slice(0, 5) ?? "");
  const [closingTime, setClosingTime] = useState(snackbar.closing_time?.slice(0, 5) ?? "");
    ...
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/70 p-4" onClick={onClose}>
      ...
      >
        ...
        <div className="space-y-3 p-5">
          ...
          <div className="grid grid-cols-2 gap-3">
            <ModalField label="Abertura" icon={<Clock size={12} />}>
              <input
                type="time"
                value={openingTime}
            ...
            <ModalField label="Fechamento" icon={<Clock size={12} />}>
              <input
                type="time"
                value={closingTime}
...
// Suppress unused import lints retained for compatibility
void ArrowRight;
